...
(ns langohr.consumers
  ...
  (:import [com.rabbitmq.client Channel Consumer DefaultConsumer
  ...
  (proxy [DefaultConsumer] [(as-non-recovering-channel channel)]
    ...
    (handleDelivery [^String consumer-tag ^Envelope envelope ^AMQP$BasicProperties properties ^bytes body]
      (when handle-delivery-fn
        (handle-delivery-fn channel
                            (merge (to-message-metadata properties)
                                   (to-message-metadata envelope))
                           ;; (to-message-metadata (QueueingConsumer$Delivery. envelope properties body))
                            body)))))
